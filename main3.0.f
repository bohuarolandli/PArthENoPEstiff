...
22	read(1,'(a)') line
	...
	elseif (line(1:7).eq.'RHOLMBD') then
	  ...
	  ierror=.false.
	  cflag='d'
	  call readline('KAPPA',valr,vall,valc,ierror)
          if (ierror) then
	    write(*,99) keyword
	    stop
          endif
...
601	write(*,6001) (byy(ixt(i)),i=1,nvxt)
6001	format(/
...
603	isel=500
	goto 804
604	write(*,'(//////////////////////)')
605	write(*,6003) inuc,(i,byy(i),i+6,byy(i+6),i+12,byy(i+12),i+18,
     .    byy(i+18),i+24,byy(i+24),i=1,2)
6003	format(//
     .    1x,'The nuclides whose abundances you can save are from 1 to',
     .    i3,' in the list below:'/
     .    2(/1x,i2,'.',a5,4(8x,i2,'.',a5)))
	write(*,6004) (i,byy(i),i+6,byy(i+6),i+12,byy(i+12),i+18,
     .    byy(i+18),i=3,6)
6004	format(
...
701	write(*,7010) eta10,inuc,dnnu0,irec,tau0,xie0,xix0,rholmbd0,kappa0
7010	format(///////////////////
     .    1x,'Settings are:'//
...
C-----Energy density of a cosmological constant
	rholmbd0=0.d0
C-----Ratio of stiff-to-photon energy density at 10 MeV
	kappa0=0.d0
...
900	eflag=0
	etaf0=1.d-10*eta10
	call parthenope(etaf0,dnnu0,tau0,xie0,xix0,rholmbd0,kappa0)
	write(*,*)
	if (eflag.eq.0) then
