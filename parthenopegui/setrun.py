...
class ConfigNuclidesOutput(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.selAllButton.clicked.connect(
            lambda x: self.fillNuclides(clear=True, addall=True)
        )
...
class MWRunPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.saveDefault.clicked.connect(lambda: self.prepareRun(default=True))
    ...
    def startRunDefault(self):
        ...
        """
        try:
            cp, gp = self.prepareRun(default=True)
        except TypeError:
            return None, None
        else:
            self.startRun()
