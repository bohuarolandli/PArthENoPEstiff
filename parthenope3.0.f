...
C
C=======================================================================
	SUBROUTINE PARTHENOPE(ETAF0,DNNU0,TAU0,XIE0,XIX0,RHOLMBD0,KAPPA0)
...
C       kappa0=ratio of stiff-to-photon energy density at 10 MeV      
C
Cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
C-----Initialization
	call init(etaf0,dnnu0,tau0,xie0,xix0,rholmbd0,kappa0,zin,zend)
...
9999	format(2x,a,i2,a,d12.5)
	...
	END
        ...
        SUBROUTINE INIT(ETAF0,DNNU0,TAU0,XIE0,XIX0,RHOLMBD0,KAPPA0,
     .       ZIN,ZEND)
Cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
C--------------------------Common variables-----------------------------
	...
	COMMON/MODPAR/   TAU,DNNU,ETAF,RHOLMBD,KAPPA,XIE,XIX
...
C-----nout has to be smaller than the dimension of outtxt
	...
	if (.not.fexist) then
	  ...
	  write(2,2002) 'DN_nu',' xie',' xix','tau','rholmbd ','kappa',
     .         'eta10','OmegaBh^2',' phie',(outtxt(i),i=1,nout),
     .         (cyy(ixt(i)),i=1,nvxt)
2002	  format((2x,a),2(3x,a),2(4x,a),4(3x,a),8x,30(a7,7x))
	else
...
C-----thetah=dr/dt=3*Hh (dimensionless Hubble function)
	thetah=dsqrt(24.*pi*gn*((me/z)**4*rhoh+rholmbd
     .       +dnnu*5.713025177928327d3/ar**4
     .       +kappa*0.6573401840810518d4/ar**6))/me
	outvar(1)=thetah
        !write(*,*) ar
...
c!!che si deve fare qui? bloccare il codice?
	if (eflag.ne.0) return
...
C	naux vanishes in the limit of thermal neutrino distribution functions
	...
	rhonh=(3.0045813575784174d-7-0.00001643531199791781d0*z+
     ...
     .       ((1.d0+1.d0*ex(z))*z**4)
C-----Compute the current *scale factor*, a/a_i.
C     taking into account of nonthermal neutrino heating
        if (z .ne. z0) then
           if (z .lt. 4) then
              faux = -1.2042903766484858d-5*z**13
     .          + 3.2461156281516864d-4*z**12
     .          -3.8960081014544415d-3*z**11+2.7454677924511183d-2*z**10
     .          -1.261195373853664d-1*z**9 + 3.965102768969565d-1*z**8
     .          -8.715943305004987d-1*z**7 + 1.3446115126715301d0*z**6
     .          -1.4396956680883248d0*z**5 +1.0400350684821418d0*z**4
     .          -4.7836047019233596d-1*z**3 + 1.2059638568939307d-1*z**2
     .          -5.2779353212949395d-3*z + 1.1513324315987712d-5
           else
              faux = 5.459941906888273d-3
           endif
           ar = z/z0 * ex(faux/4.d0)
     .          * (1.7272681902205032d0
     .          / rhonh**(1d0 + .25d0*naux/rhonh))**0.25d0
           !write(*,*) z/z0, faux, scale_factor
        else
           ar = 1.d0
        endif
        outvar(4) = ar
...
C	rhonh=rhonh+(7./8.d0)*2.*(dnnu+dnnuxi)*(pi**2/30.)*tnuh**4
C-----------------------------------------------------------------------
...
C.......H(n,g)H2...................(Ando et al. 2006)
      if (t9.le.1.5d0) then
         f(12) = 44216.*(1.d0 + 3.75191*t9 + 1.92934*t9**2 +
     .        0.746503*t9**3 + 0.0197023*t9**4 +
     .        3.00491d-6*t9**5) / (1.d0 + 5.4678*t9 + 5.62395*t9**2 +
     .        0.489312*t9**3 + 0.00747806*t9**4)
         
         df = 0.00449213 * (1.d0 + 3.08947*t9 + 0.13277*t9**2 +
     .        1.66472*t9**3) / (1.d0 + 2.75245*t9 + 1.40958*t9**2 +
     .        0.8791*t9**3)
      else
...
c$$$	if (t9.le.1.5d0) then
c$$$	  f(12)=44060.*(1.d0 - 2.7503695564153143*t9 - 
c$$$     -    3.5220409333117897*t9**2 - 0.2093513619089196*t9**3 + 
c$$$     -    0.10659679579058313*t912 + 4.62948586627009*t932 + 
c$$$     -    1.3459574632779876*t952)
c$$$	  df=67.95212182165885*dsqrt(1.255364135304537d0 + 
c$$$     -    618.8966688079934*t9 + 21793.605102078516*t9**2 + 
c$$$     -    97573.42092925594*t9**3 + 85539.62453206794*t9**4 + 
c$$$     -    14687.125326371255*t9**5 + 247.57751149254238*t9**6 - 
c$$$     -    2855.9302137640752*t9112 - 25.82889427679147*t912 - 
c$$$     -    5043.279674275595*t932 - 57307.155278753264*t952 - 
c$$$     -    111071.90152148822*t972 - 44154.571125115704*t992)
c$$$	else
c$$$	  f(12)=4.742d4*(1.d0-.8504d0*t912+.4895d0*t9-.09623d0*t932+
c$$$     -    8.471d-3*t9**2-2.80d-4*t952)	!SKM
c$$$	endif
c$$$	if (nchrat.ne.0) then
c$$$	  if (t9.le.1.5d0) then
c$$$	    drate(12,1)=-df/f(12)
c$$$	  else
c$$$	    drate(12,1)=-.078d0
c$$$	  endif
c$$$	  drate(12,2)=-drate(12,1)
c$$$	endif
	  ...
	  do i=2,irec
	    if (hchrat(i).gt.0) then
	      if (hchrat(i).eq.3) then
                 !f(i)=factor(i)*f(i)
                 if (factor(i)<0) then
                    f(i) = f(i) * (1.d0+drate(i,1))**dabs(factor(i))
                 else
                    f(i) = f(i) * (1.d0+drate(i,2))**factor(i)
                 endif
	      else
...
C--------------------------Common variables-----------------------------
	...
	DIMENSION        YYOLD(3,NNUC+1)
!        DIMENSION        TMPVAR(3,5)
        DIMENSION        TMPVAR(3,6)
	COMMON/YOLD/     YYOLD,TMPVAR
C-----------------------------------------------------------------------

        !write(*,*) zend, ioutev, outvar(4)
C-----Counter increment
	ioutev=ioutev+1
...
C-----We make the check only for the nuclides to be followed
	  ...
	  if (follow) then
	    ...
	    if (ncount.eq.24) then
	      write(*,*) '   DN_nu    xie    xix  tau  rholmbd  kappa',
     .              '   eta10       T(MeV)         Y_D         Y_He4',
     .              '        Y_Li7'
	      ncount=0
	    endif
	    write(*,15) dnnu,xie,xix,tau,rholmbd,kappa,etaf*1.d10,
...
15	format(1x,5f7.2,f7.3,f8.5,4e13.5)
...
C-----Final output
	...
	abund(6)=yp
	abund(8)=lithium
	write(4,'(2(4x,a,f8.5)/2(4x,a,f7.3))')
     .       'eta10 = ',etaf*1.d10,'Omega_B h^2 = ',
C     .       etaf*1.d10*(1.d0-0.007125*yp)/273.279,
     .       etaf*1.d10/(1.d0+7.16958d-3*yp)/273.3036,
     .    'Rholmbd = ',rholmbd,'kappa = ',kappa        
	write(4,*)
	if (ixt(30).eq.1) then
	  write(3,16) me/zend,yy(1),(outvar(i),i=1,nout),
     .      (yy(ixt(i)+1),i=1,nvxt)
	endif
	write(2,17) dnnu,xie,xix,tau,rholmbd,kappa,etaf*1.d10,
C     .       etaf*1.d10*(1.d0-0.007125*yp)/273.279,
     .       etaf*1.d10/(1.d0+7.16958d-3*yp)/273.3036,
     .       yy(1),(outvar(i),i=1,nout),(abund(ixt(i)),i=1,nvxt)
        !write(*,*) nout, outvar(2), outvar(4)

14	format(13x,a,2x,2(d12.5,a))
15	format(11x,2a,2x,2(d12.5,a))
!16     format(1x,d10.5,48d13.5)
16	format(1x,d12.7,48d15.7)
17	format(1x,f6.3,2f7.2,f8.2,e11.3,f8.3,2f9.5,48d14.6)
